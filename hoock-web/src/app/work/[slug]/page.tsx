...
// Ensure safe image URL extraction
...
const extractGalleryUrls = (galleryProp: any): string[] => {
  ...
  if (Array.isArray(galleryProp)) {
    rawArray = galleryProp;
  } else if (galleryProp.data && Array.isArray(galleryProp.data)) {
    rawArray = galleryProp.data;
  }

  return rawArray.map((img: any) => {
    let url = img.url || img.attributes?.url;
    if (!url) return "https://picsum.photos/1200/600";
    return url.startsWith('http') ? url : `${STRAPI_URL}${url}`;
  });
};
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center pt-32 pb-24 text-gray-400">
        <span className="inline-block animate-spin border-4 border-gray-200 border-t-black rounded-full w-12 h-12 mb-4"></span>
      </div>
    );
  }
  ...
  if (!work) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-8 text-[#1C2329] pt-32">
        ...
        <Link href="/#works" className="px-6 py-3 bg-[#1C2329] text-white font-semibold rounded hover:bg-[#D9A384] transition-colors">
          Return Home
        </Link>
      </div>
    );
  ...
  return (
    <article className="min-h-screen bg-white pb-24 pt-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Back Link */}
        <Link href="/#works" className="inline-flex items-center text-sm font-medium text-gray-500 hover:text-[#D9A384] mb-8 transition-colors uppercase tracking-widest">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentImages.map((src, idx) => (
                ...
                >
                  ...
                  />
                  {/* Overlay on hover indicating it's clickable */}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-6 mt-12">
                <button 
                  ...
                  className="p-2 rounded-full border border-gray-300 disabled:opacity-30 hover:bg-gray-100 transition-colors"
                >
      ...
      {/* Lightbox / Popup Fullscreen Image */}
      {selectedPhoto && (
        <div 
        ...
        >
          ...
          <div className="relative w-full max-w-6xl max-h-[90vh] aspect-video sm:aspect-auto sm:h-full flex items-center justify-center pointer-events-none">
             {/* The image itself allows pointer events so we only close when clicking outside */}
            <Image 
              src={selectedPhoto} 
              alt="Popup detail" 
              fill
